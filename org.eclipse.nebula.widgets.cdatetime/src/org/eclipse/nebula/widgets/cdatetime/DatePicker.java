	...
	protected boolean setFocus(boolean focus) {
		if(!focus) {
			...
			if(cdt.hasSelection()) {
				...
				Date selection = (Date) cdt.getSelection(Date.class);
				// TODO range and multi-select
...
//				}
	...
	void setMonthLabelText() {
		String str = getFormattedDate("MMMM", cdt.getReferenceTime()); //$NON-NLS-1$
		GC gc = new GC(getDisplay());
		int width = monthButton.getClientArea().width;
		if(width > 0 && gc.stringExtent(str).x >= width) {
			str = getFormattedDate("MMM", cdt.getReferenceTime()); //$NON-NLS-1$
		}
		gc.dispose();
...
//				}
	...
	private void updateHeader() {
		if(header != null) {
			...
			if(yearButton != null) {
				yearButton.setText(getFormattedDate("yyyy", cdt.getReferenceTime())); //$NON-NLS-1$
			}
	...
	void updateView() {
		updateHeader();
		if(dayPanel != null) {
			setDays(cdt.getReferenceTime(), true);
			updateDays();
		}
		updateMonths();
		updateYears();
